...
@Component({
    ...
    styleUrls: ['./ng-swipeable-panel.component.scss'],
})
    ...
    implements AfterViewInit
{
    ...
    public ngAfterViewInit(): void {
        const containerHeight =
            this.swipeableContainer.nativeElement.clientHeight;
        this.maxHeight =
            containerHeight >= this.maxStaticHeight
                ? this.maxStaticHeight
                : containerHeight;
            ...
            fromEvent(window, 'scroll')
                ...
                .subscribe((scrollTop) => {
                    if (hidingElement) {
                        ...
                        const elementBottom =
                            hidingElement.offsetTop +
                            hidingElement.offsetHeight;
                        const elementVisible =
                            windowBottom > hidingElement.offsetTop &&
    ...
    private isHalfAboveOrBelow(
        halfPoint: 'above' | 'below',
        currentPosition: number,
    ): boolean {
        const maxHeightHalf = this.maxHeight / 2;
        ...
        if (this.fullscreen && this.platformService.isBrowser()) {
            merge(fromEvent(window, 'popstate'), fromEvent(window, 'click'))
                ...
                .subscribe((event) => {
                    ...
                    if (
                        isPointerEvent &&
                        !this.swipeableContainer.nativeElement.contains(
